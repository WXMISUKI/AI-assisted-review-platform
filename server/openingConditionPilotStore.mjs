...
function buildReportExportRowsFromFindings(findings = []) {
  return findings.slice(0, 120).map((finding, index) => ({
    ...
    riskLevel: getReportRiskLabel(finding.riskLevel),
    dispositionLabel: getReportFindingLabel(finding),
    basis: finding.basisVersionId || "\u672a\u8bb0\u5f55",
    rectificationRequirement: finding.rectificationRequirement,
  }));
}
    ...
    .map(
      (row) => `
        <tr>
          ...
          <td>${reportHtmlText(row.riskLevel, 120)}</td>
          <td>${reportHtmlText(row.dispositionLabel, 120)}</td>
          <td>${reportHtmlText(row.issueDescription, 800)}</td>
          <td>${reportHtmlText(row.basis, 300)}</td>
          <td>${reportHtmlText(row.rectificationRequirement, 800)}</td>
        </tr>`,
    )
    .join("");
  ...
  <body>
    ...
    <table>
      <tr>
        ...
        <th style="width: 21%;">问题描述</th>
        <th style="width: 12%;">依据</th>
        <th style="width: 19%;">整改要求</th>
      </tr>
      ${findingRows || "<tr><td colspan=\"8\">当前没有结构化问题项。</td></tr>"}
    </table>
