...
test("UI smoke keeps the opening-condition agent detail workbench usable", async () => {
  ...
  assert.match(source, /function buildOpeningConditionAgentEvidenceSummaries/);
  assert.match(source, /function buildOpeningConditionAgentDecisionLedgerStatus/);
  assert.match(source, /activeEvidenceSummaries/);
  assert.match(source, /activeDecisionLedgerStatus/);
  assert.match(source, /canSubmitActiveReviewDecision/);
  assert.match(source, /关联证据摘要/);
  assert.match(source, /人工复核决策账本/);
  assert.match(source, /无 provider 交接记录/);
  assert.match(source, /当前项没有可提交的新决策/);
  assert.match(source, /!activeDecisionLedgerStatus\.actionable \|\| pilotBusy/);
  assert.match(source, /opening-agent-evidence-card/);
  assert.match(source, /opening-agent-decision-ledger-card/);
  assert.match(styles, /\.opening-agent-evidence-card/);
  assert.match(styles, /\.opening-agent-evidence-row/);
  assert.match(styles, /\.opening-agent-decision-ledger-card/);
