...
平台后端通过 `KNOWLEDGE_PROVIDER` 选择知识库 provider。当前试点的推荐边界是：平台调用 OCR Worker / MaxKB Provider Proxy，由 Proxy 内部处理 MaxKB 登录和转发；平台不直连 MaxKB 容器，也不保存 MaxKB 管理员账号密码。
...
`MAXKB_API_KEY` 对应 Worker/Proxy 的 Bearer key。`MAXKB_USERNAME` / `MAXKB_PASSWORD` 只应配置在 Worker/Proxy 运行环境内；平台侧保留这些字段仅用于历史兼容，不作为本试点推荐配置。
...
readiness 优先读取 `MAXKB_STATUS_PATH`，并兼容 `MAXKB_HEALTH_PATH` 返回的 `providers.maxkb` 嵌套摘要。平台只展示 configured、ready、status、summary、workspaceId、defaultKnowledgeId 和 capabilities 等安全字段。
