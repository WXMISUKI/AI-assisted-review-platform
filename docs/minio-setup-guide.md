...
docker run -d \
  ...
  -e MINIO_ROOT_PASSWORD=minio123@jtjl \
...
# 访问控制台
# 浏览器打开: http://192.168.1.85:9001
# 用户名: miniominio
# 密码: minio123@jtjl

# 健康检查
curl http://192.168.1.85:9000/minio/health/live
```
...
```bash
# 添加 MinIO 服务别名
mc alias set myminio http://192.168.1.85:9000 miniominio minio123@jtjl
...
```bash
# 创建 Bucket
mc mb myminio/http-tools

# 设置为公开读取（可选，适用于需要公开访问的场景）
mc anonymous set download myminio/http-tools
...
```bash
# 查看当前策略
mc anonymous get myminio/http-tools

# 设置公开读取
mc anonymous set download myminio/http-tools

# 恢复私有
mc anonymous set none myminio/http-tools
```
...
```env
# MinIO 内网访问地址
MINIO_ENDPOINT=http://192.168.1.85:9000
...
MINIO_SECRET_KEY=minio123@jtjl

# Bucket 名称
MINIO_BUCKET=http-tools
...
```bash
# 检查凭证是否正确
mc alias set test http://192.168.1.85:9000 miniominio minio123@jtjl
mc ls test
...
# 查看 Bucket 内文件
mc ls myminio/http-tools

# 上传文件
mc cp ./local-file.txt myminio/http-tools/

# 下载文件
mc cp myminio/http-tools/remote-file.txt ./

# 删除文件
mc rm myminio/http-tools/file.txt

# 查看 Bucket 使用情况
mc du myminio/http-tools
