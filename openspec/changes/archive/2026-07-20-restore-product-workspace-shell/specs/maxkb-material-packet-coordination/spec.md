...
- **THEN** the provider accepts platform identifiers, idempotency key, source object refs, file metadata, document category hints, and correlation id, then returns per-file processing status and safe provider document refs
